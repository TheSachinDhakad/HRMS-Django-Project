...
from .models import Department,Kin,Attendance, Leave, Recruitment,User
...
class EmployeeForm (UserCreationForm):
    ...
    Account_number = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control','placeholder':'Enter your A/C No.'}))
    IFSC_Code = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Enter IFSC Code'}))
    Bank_Name = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'Enter BankName'}))
    class Meta:
        model = get_user_model()
        fields = ('username','email','password1', 'password2','thumb', 'first_name', 'last_name', 'mobile','Gender',
                   'department','salary','Designation','Date_of_joining','Bank_Name','Account_number','IFSC_Code')
